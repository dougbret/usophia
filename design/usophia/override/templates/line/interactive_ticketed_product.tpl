{* Interactive ticketed Product  *}



{def 
     $interactive_ticketed_product = ''
     $interactive_object = ''
     $interactive_ticketed_item = ''
  ...
  $help_text_click_to_buy_interactive = 'Click to buy to join the full event'
  ...
  $event_type = ''
   ...
   {set $interactive_ticketed_product = fetch( 'content', 'node', hash( 'node_id', $node.node_id))}
   {set $interactive_object = fetch( content, object, hash( object_id, $interactive_ticketed_product.contentobject_id ))}  
...
{debug-log var=$interactive_object msg='$interactive_object'}
{debug-log var=$interactive_ticketed_item msg='$interactive_ticketed_item'}
...
{debug-log  msg='case 1'}
     ...
     {else}
           {set $help_text = $help_text_click_to_buy_interactive}
     {/if}  
...
{debug-log  msg='end of switch'}
   ...
   {if $interactive_on|not}     
         <div class="block1 ">
           <div class="header">
             <h2>Auditorium - Interactive Event - {$item_node.name}</h2>
           </div>
   ...
   {if $interactive_on}
          <div class="block1">
            ...
            <div class="interactive-preview">
              <img src="{$item_node.data_map.preview_image.content['player_preview'].url|ezroot(no)}"/>
           {if $event}
                {set $object = fetch( content, object, hash( object_id, $event.0.id ))} 
             ...
             <div class="action-buttons">
              {if $event}
                {if $event.0.data_map.link.has_content}
                      ...
                      {set $interactive_url = concat('"',$meeting_url[0],'//',$meeting_url[2],'/',$meeting_url[3],'/m.php?AT=',$event_type,'&MK=',$object.main_node.data_map.meeting_id.content|trim,'&AN=',$user.login,'&AE=',$user.email,'&FN=',$user.contentobject.data_map.first_name.content,'&LN=',$user.contentobject.data_map.last_name.content,'&BU=http://', $hostname, '/', $item_node.url_alias|ezurl,'"')}
