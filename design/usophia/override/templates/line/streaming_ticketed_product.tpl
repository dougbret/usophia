{* Streaming ticketed Product  *}



{def $streaming_ticketed_product = ''
     $streaming_object = ''
     $streaming_ticketed_item = ''
  ...
  $event_type = ''
   ...
   {set $streaming_ticketed_product = fetch( 'content', 'node', hash( 'node_id', $node.node_id))}
   {set $streaming_object = fetch( content, object, hash( object_id, $streaming_ticketed_product.contentobject_id ))}  
...
{debug-log var=$streaming_object msg='$streaming_object'}
{debug-log var=$streaming_ticketed_item msg='$streaming_ticketed_item'}
...
{debug-log  msg='case 0'}
     {if $has_streaming_ticket_access}
	    {set $purchased = true()}
           {set $purchased_text = 'Purchased Live event'}
     {/if}
     {set $preview = 'Trailer'}
     {if $item_node.data_map.streaming_preview_url.has_content|not}
           {set $help_text = concat($help_text_no_trailer, $help_text_Live_still_to_come)}
     {else}          
        {set $streaming_url = $item_node.data_map.streaming_preview_url.content|trim}
...
{debug-log  msg='case 1'}
     ...
     {/if}
        ...
        {if $has_streaming_ticket_access}
           ...
           {set $streaming_url = $item_node.data_map.url.content|trim}
           {set $os_load=true()}
        {elseif $item_node.data_map.streaming_preview_url.has_content|not}
           {set $preview = 'Trailer'}
           {set $help_text_login = $help_text_view_full}
           {set $login_flag = true()}
           {set $help_text = concat($help_text_no_trailer, $help_text_Live_current)}
           {set $permissions_flag=true()}
        {else}
           {set $preview = 'Trailer'}
           {set $help_text = $help_text_Live_current}
           {set $help_text_login = $help_text_current}
           {set $streaming_url = $item_node.data_map.streaming_preview_url.content|trim}
           {set $os_load=true()}
...
{debug-log  msg='end of switch'}
      ...
      {if and($login_flag, $permissions_flag|not)}
         {if $help_text.has_content}
           {set $help_text = concat($help_text, ' ', $login_text, $help_text_login)}
         {else}
           {set $help_text = concat($login_text, $help_text_login)}
         {/if}
      {elseif and($login_flag, $permissions_flag)}
         {if $help_text.has_content}
           {set $help_text = concat($help_text, ' ', $login_text, $help_text_login_and_click_to_buy)}
         {else}
           {set $help_text = concat($login_text, $help_text_login_and_click_to_buy)}
         {/if}
      {elseif and($login_flag|not, $permissions_flag)}
         {if $help_text.has_content}
           {set $help_text = concat($help_text, $help_text_click_to_buy)}
         {else}
