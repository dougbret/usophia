...
<div class="content-view-line" id="slide-item-{$node.node_id}">
  ...
  {if ne($node.parent.node_id,2)}
    ...
    {/if}
  {else}
    <div class="banner-image art-center-banner-line" style="background:url({$node.data_map.background_image.content['original'].url|ezroot(no)}) no-repeat right 0; color:{$node.data_map.text_color.content}; height:247px">
  {/if}
            ...
            <script type="text/javascript">
               ...
               // var video = $(this).parent().parent().find('.content-player').html();
                ...
                $("#player-link-{/literal}{$node.node_id}{literal} a").fancybox(
                  {
                        'content':'<div id="player-pop-up"></div>',
                          ...
                          onComplete: function() {
                              window.player_id = "player-pop-up";
