...
<form name="versionsform" action={concat( '/content/history/', $object.id, '/' )|ezurl} method="post">
    ...
    <h1>{'Versions for <%object_name> [%version_count]'|i18n( 'design/ezwebin/content/history',, hash( '%object_name', $object.name, '%version_count', $list_count ) )|wash}</h1>
...
{if and( is_set( $object ), is_set( $diff ), is_set( $oldVersion ), is_set( $newVersion ) )|not}
    ...
    <h1>{'Published version'|i18n( 'design/ezwebin/content/history' )}</h1>
...
{undef $initial_language}
    ...
    <h1>{'New drafts [%newerDraftCount]'|i18n( 'design/ezwebin/content/history',, hash( '%newerDraftCount', $newerDraftVersionListCount ) )}</h1>
...
{/literal}
    ...
    <h1>{'Differences between versions %oldVersion and %newVersion'|i18n( 'design/ezwebin/content/history',, hash( '%oldVersion', $oldVersion, '%newVersion', $newVersion ) )}</h1>
